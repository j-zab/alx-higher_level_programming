...
def matrix_mul(m_a, m_b):
    ...
    Raises:
        ...
        TypeError: If one element of list of lists isn't int/float
        TypeError: If row of m_a or m_b aren't the same size
    ...
    if not all((isinstance(element, int) or isinstance(element, float))
        ...
        raise TypeError("m_a should contains only integers or floats")
    if not all((isinstance(element, int) or isinstance(element, float))
            for element in [number for row in m_b for number in row]):
        raise TypeError("m_b should contains only integers or floats")
